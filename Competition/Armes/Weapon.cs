...
namespace Competition.Armes
{
    abstract class Weapons //Classe abstraite utilisé comme template pour toutes les armes
    {
        ...
        public abstract Vector2 MouseDirection { set; } //propriété utilisé pour mettre a jours la direction pinté par la sourie
    ...
    sealed class Pistol : Weapons
    {
        ...
        public override void MouseDown(Vector2 MouseDir)
        {
            if (m_CanShoot)
            {
                ...
                MouseDir = new Vector2((float)Math.Cos(Radians), (float)Math.Sin(Radians));
